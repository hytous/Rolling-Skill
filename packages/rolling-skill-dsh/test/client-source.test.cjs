...
describe("Rolling Skill native DSH Client", () => {
    ...
    it("promotes Skill import, version management, and installation to second-level pages", () => {
        ...
        assert.match(skills, /skills\.candidateBase/u)
        assert.match(skills, /interface CandidateBase/u)
        assert.match(skills, /version\.contentDigest === candidateBase\.contentDigest/u)
        assert.match(skills, /version\.contentDigest === expectedBase\.contentDigest/u)
        assert.match(skills, /disabled=\{busy \|\| !candidateBase \|\| contentAlreadyRecorded\}/u)
        assert.match(skills, /contentAlreadyRecorded \? t\("versionUnchanged"\) : t\("prepareVersionDescription"\)/u)
        assert.doesNotMatch(skills, /candidateMessage|versionChangeNote/u)
        assert.match(locale, /publishedVersions:\s*"已发布版本"/u)
        assert.match(locale, /prepareVersion:\s*"准备发布"/u)
        assert.match(locale, /prepareVersionDescription:\s*"检测到 Skill 内容有变更/u)
        assert.match(locale, /versionUnchanged:\s*"当前 Skill 内容与已记录版本一致/u)
        assert.match(locale, /versionReady:\s*"当前内容已准备好，填写版本号即可发布。"/u)
        assert.match(locale, /saveVersionContent:\s*"保存为待发布版本"/u)
        assert.match(locale, /releaseVersion:\s*"发布新版本"/u)
        ...
        assert.match(source("workbench/workbench.css"), /\.rolling-skill-version-workflow/u)
    })
