...
describe("Rolling Skill native DSH Client", () => {
    ...
    it("projects durable Draft and saved ranges onto native flow keys", () => {
        ...
        const sharedMarkers = source("conversation/useConversationMarkers.ts")
        ...
        assert.match(sharedMarkers, /conversationCuration\.markers/u)
        assert.match(controller, /useConversationMarkers/u)
        assert.match(controller, /useSession/u)
        assert.match(controller, /data-chat-flow-key/u)
        assert.match(controller, /CSS\.escape/u)
        assert.match(sharedMarkers, /rolling-skill:curation-markers-changed/u)
    ...
    it("groups workbench pages into four workflow sections", () => {
        ...
        assert.match(workbench, /defaultPage:\s*"skill-import"/u)
        assert.match(workbench, /\{id:\s*"skill-import",\s*label:\s*"skillImportTab"\}/u)
        assert.match(workbench, /\{id:\s*"skill-versions",\s*label:\s*"skillVersionsTab"\}/u)
        assert.match(workbench, /\{id:\s*"skill-install",\s*label:\s*"skillInstallTab"\}/u)
        assert.doesNotMatch(workbench, /\{id:\s*"skills"|\{id:\s*"installations"/u)
    ...
    it("ships native Dataset, Case, and Raw Case panels with refresh and recovery actions", () => {
        ...
        // Multiline notes are exercised by raw-case-review-journey.test.cjs.
        assert.doesNotMatch(rawCases, /<Input\s+value=\{note\}/u)
        assert.match(locale, /searchRawCases:\s*"搜索问题或备注"/u)
        assert.match(locale, /searchRawCases:\s*"Search questions or notes"/u)
        assert.doesNotMatch(locale, /搜索问题、备注或 Skill|Search questions, notes, or Skills/u)
    ...
    it("keeps evaluation Runtime identity details but makes Case refresh a compact single-select", () => {
        ...
        // Per-Runtime model/effort payloads are exercised by install-evaluate-journey.test.cjs.
    ...
    it("consolidates managed Skill release, Runtime installation, and installation audit", () => {
        ...
        assert.doesNotMatch(skills, /installationJobs/u)
        assert.match(skills, /AgentProfileControls/u)
        assert.match(skills, /targets:\s*runtimeIds\.map\(\(selectedRuntimeId\)\s*=>\s*\(\{/u)
        assert.match(skills, /modelId:\s*runtimeProfiles\[selectedRuntimeId\]\?\.modelId\s*\|\|\s*null/u)
        assert.match(skills, /effort:\s*runtimeProfiles\[selectedRuntimeId\]\?\.effort\s*\|\|\s*null/u)
        assert.doesNotMatch(skills, /targets:[^\n]+effort:\s*"high"/u)
        assert.match(workbench, /<SkillsPanel/u)
        assert.doesNotMatch(workbench, /<InstallationsPanel/u)
        assert.match(skills, /<InstallationsPanel/u)
        assert.match(installations, /installations\.list/u)
        assert.match(installations, /installations\.get/u)
        assert.match(installations, /installations\.cancel/u)
        assert.match(installations, /installations\.send/u)
    ...
    it("promotes Skill import, version management, and installation to second-level pages", () => {
        const skills = source("workbench/SkillsPanel.tsx")
        const workbench = source("workbench/Workbench.tsx")
        const locale = source("locale.ts")

        assert.match(skills, /mode:\s*"import" \| "versions" \| "install"/u)
        assert.match(skills, /mode === "import"/u)
        assert.match(skills, /mode === "versions"/u)
        assert.match(skills, /mode === "install"/u)
        assert.doesNotMatch(skills, /SkillManagementTab|skillManagementSections|rolling-skill-skill-tabs/u)
        assert.match(skills, /value=\{detail\?\.skill\.id \?\? ""\}/u)
        assert.match(skills, /initialSkillId \?\? detail\?\.skill\.id/u)
        assert.doesNotMatch(skills, /if \(!detail && requestedSkill\)/u)
        assert.match(workbench, /page:\s*"skill-import"/u)
        assert.match(workbench, /page:\s*"skill-versions"/u)
        assert.match(workbench, /page:\s*"skill-install"/u)
        assert.match(workbench, /candidate\.page === "skills"/u)
        assert.match(workbench, /candidate\.page === "installations"/u)
        ...
        assert.match(skills, /detail\?\.versions\.filter\(\(version\) => version\.state === "released"\)/u)
        assert.doesNotMatch(skills, /detail\.versions\.map|t\("candidateVersion"\)|t\("createCandidate"\)/u)
        assert.doesNotMatch(skills, /skills\.candidateBase|interface CandidateBase|contentAlreadyRecorded/u)
        assert.match(skills, /<SkillEditModal/u)
        assert.match(locale, /publishedVersions:\s*"已发布版本"/u)
        assert.match(locale, /editSkillWithAgent:\s*"让 Agent 编辑"/u)
        assert.match(skills, /<details className="rolling-skill-manifest-details">/u)
        assert.match(locale, /viewSkillContent:\s*"查看 Skill 内容"/u)
        assert.doesNotMatch(locale, /candidateVersion:\s*"候选版本"|createCandidate:\s*"创建候选版本"/u)
        assert.doesNotMatch(skills, /saveVersionContent|releaseLabel|createCandidate/u)
    })

    it("edits one managed Skill through an Agent draft and automatically publishes the result", () => {
        const skills = source("workbench/SkillsPanel.tsx")
        const modal = source("workbench/SkillEditModal.tsx")
        const locale = source("locale.ts")
        const css = source("workbench/workbench.css")

        assert.match(skills, /skills\.path/u)
        assert.match(skills, /skills\.revealSkill/u)
        assert.match(skills, /copyManagedSkillPath/u)
        assert.match(skills, /<Button size="sm" disabled=\{busy\} onClick=\{\(\) => setEditOpen\(true\)\}/u)
        assert.doesNotMatch(skills, /<Button tone="primary" size="sm" disabled=\{busy\} onClick=\{\(\) => setEditOpen\(true\)\}/u)
        assert.match(modal, /skillEdits\.list/u)
        assert.match(modal, /skillEdits\.start/u)
        assert.match(modal, /skillEdits\.get/u)
        ...
        assert.match(modal, /<select className="rolling-skill-select" aria-label=\{t\("skillEditRuntime"\)\}/u)
        assert.doesNotMatch(modal, /<RuntimeSelect/u)
        assert.match(modal, /<ModelEffortSelect/u)
        assert.match(modal, /RuntimeInteractions/u)
        assert.match(modal, /localStorage/u)
        assert.match(modal, /setInterval[\s\S]{0,300}1_500/u)
        assert.match(modal, /diff\?\.files/u)
        assert.doesNotMatch(modal, /workspacePath|skillRoot|baseCommit|baseContentDigest/u)
        assert.match(locale, /applySkillEdit:\s*"应用修改并发布新版本"/u)
        assert.match(locale, /discardSkillEdit:\s*"放弃草稿"/u)
        assert.match(css, /\.rolling-skill-skill-edit-layout/u)
        assert.match(css, /\[role="dialog"\]:has\(> div > div > \.rolling-skill-skill-edit-modal\)\s*\{[\s\S]*max-width:\s*1120px/u)
        assert.match(css, /\[role="dialog"\]:has\(> div > div > \.rolling-skill-skill-edit-modal\) > div:first-child > div:last-child\s*\{[\s\S]*overflow-y:\s*auto/u)
        assert.doesNotMatch(css, /\.rolling-skill-skill-edit-modal\s*\{[\s\S]{0,240}width:\s*min\(80vw/u)
        assert.match(css, /@media \(max-width: 960px\)[\s\S]*\.rolling-skill-skill-edit-layout/u)
    })
    ...
    it("ships native Operator and Optimization panels without private control fields", () => {
        ...
        assert.match(operator, /<select[^>]+value=\{runtimeId\}/u)
        assert.match(operator, /<OperatorScopeControls/u)
    ...
    it("ships the complete automatic capture settings and status panel", () => {
        ...
        assert.match(automatic, /automaticSourceRuntime/u)
    ...
    it("explains automatic model roles and background scheduling in user language", () => {
        ...
        assert.match(locale, /disableScheduler:\s*"停止后台定时运行"/u)
    ...
    it("makes automatic capture routing and scheduler actions explicit", () => {
        ...
        assert.doesNotMatch(automatic, /setModels\(\[\]\)[\s\S]{0,120}setModelId\(""\)/u)
