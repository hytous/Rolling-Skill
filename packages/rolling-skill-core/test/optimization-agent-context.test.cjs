...
const {optimizationRequestMessage} = require("../src/optimization-agent-context.cjs")
...
test("the optimization Agent receives bounded Case evidence rather than only a run id", () => {
    ...
    const message = optimizationRequestMessage({run: {id: "run-1", snapshot: {limits: {maxEpochs: 1}, target: {minimumScore: 90}}}, kind: "candidate", epoch: 1, baselineEvaluation: baseline, currentEvaluation: baseline})
    ...
    const decision = optimizationRequestMessage({run: {id: "run-1", snapshot: {limits: {maxEpochs: 1}, target: {minimumScore: 90}}}, kind: "decision", epoch: 1, baselineEvaluation: baseline, currentEvaluation: {...baseline, id: "candidate"}})
