...
describe("local-first desktop surface", () => {
    it("provides a dedicated three-column Operator self-operation surface", () => {
        ...
        assert.match(html, /class="operator-field operator-objective-field" data-step="01"/)
        assert.match(html, /class="operator-setup-section operator-environment-section" data-step="02"/)
        assert.match(html, /class="operator-setup-section operator-resource-section" data-step="03"/)
        ...
        assert.match(styles, /\.operator-workbench\s*\{[^}]*background:\s*var\(--sidebar\)/s)
        ...
        assert.match(styles, /\.operator-setup-section::before\s*\{[^}]*content:\s*attr\(data-step\)/s)
        assert.match(styles, /\.operator-setup-footer\s*\{[^}]*position:\s*sticky/s)
