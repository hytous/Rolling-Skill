...
describe("local-first desktop surface", () => {
    it("provides a dedicated three-column Operator self-operation surface", () => {
        ...
        assert.match(html, /id="operator-objective-label"/)
        assert.match(html, /id="operator-objective-help"/)
        ...
        assert.match(renderer, /operatorOptimizationDirection:\s*"优化方向（可选）"/)
        assert.match(renderer, /operatorOptimizationDirectionHelp:\s*"留空时，系统根据基线评测和用户使用体验自主寻找改进点；填写后优先处理该方向。"/)
        ...
        assert.match(operator, /objective\.required\s*=\s*!optimization/u)
        assert.match(operator, /optimizationDirection:\s*objective\.value\.trim\(\)\s*\|\|\s*null/u)
        assert.match(operator, /view\.optimizationDirection/u)
        assert.match(operator, /view\.playbook/u)
    ...
    it("keeps automatic capture disabled and exposes episode curation controls", () => {
        ...
        assert.doesNotMatch(html, /id="change-case-dataset-skill"/)
        assert.doesNotMatch(html, /id="case-issue-description"[^>]*readonly/)
        assert.doesNotMatch(html, /id="case-issue-description"[^>]*required/)
        assert.match(renderer, /function managedSkillSelectionKey\(/)
        assert.match(renderer, /function populateManagedSkillSelect\(/)
        assert.match(renderer, /function managedSkillBindingFromSelect\(/)
        assert.match(renderer, /populateManagedSkillSelect\(elements\.newDatasetSkill/)
        assert.match(renderer, /elements\.confirmSaveCase\.textContent = t\(inProgress \? "startingCuration" : "startCuration"\)/)
        assert.match(renderer, /elements\.confirmSaveCase\.setAttribute\("aria-busy", "true"\)/)
        assert.doesNotMatch(renderer, /changeCaseDatasetSkill:/)
