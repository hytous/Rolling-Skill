...
describe("local-first desktop surface", () => {
    ...
    it("keeps automatic capture disabled and exposes episode curation controls", () => {
        ...
        assert.doesNotMatch(html, /id="case-question"[^>]*required/)
        ...
        assert.match(createHandler, /const datasetQuestion = elements\.caseQuestion\.value\.trim\(\)/)
        assert.match(createHandler, /selection\.datasetQuestionDirty && datasetQuestion/)
