...
describe("desktop main/preload bridge", () => {
    ...
    it("bridges scheduled capture status and manual Draft creation without exposing scan cursors", async () => {
        ...
        const settingsUpdateStart = main.indexOf('ipcMain.handle("settings:update"')
        const settingsUpdateEnd = main.indexOf("\n    ipcMain.handle", settingsUpdateStart + 1)
        const settingsUpdate = main.slice(settingsUpdateStart, settingsUpdateEnd)
        assert.match(settingsUpdate, /automaticCaptureSettingsChanged/u)
        assert.match(settingsUpdate, /configurationChanged/u)
        ...
        const obsoleteErrorCleanup = ready.indexOf("await automaticCaptureManager.clearObsoleteRouteError().catch(() => false)")
        assert.ok(obsoleteErrorCleanup >= 0)
        assert.ok(obsoleteErrorCleanup < ready.indexOf("createWindow()"))
        assert.ok(obsoleteErrorCleanup < ready.indexOf("automaticCaptureManager.start()"))
