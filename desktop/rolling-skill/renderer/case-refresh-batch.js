;(function exposeCaseRefreshBatch(root, factory) {
    const api = factory()
    if (typeof module === "object" && module.exports) module.exports = api
    else root.RollingSkillRefreshBatch = api
})(typeof globalThis === "undefined" ? this : globalThis, function createCaseRefreshBatch() {
    "use strict"
    ...
    class CaseRefreshBatch {
        constructor({id = `refresh-batch-${Date.now()}`, datasetId, caseIds}) {
        ...
        attachSession(caseId, sessionId) {
            ...
            this.currentSessionId = nonEmpty(sessionId, "Refresh session id")
            return true
        }
        ...
        completeAutoArchive(sessionId) {
            if (!this.archiving || sessionId !== this.currentSessionId || !this.currentCaseId) {
                throw new Error("The refresh archive does not belong to this batch")
            }
        ...
        fail(error) {
            ...
            this.status = "failed"
            this.archiving = false
            this.error = error?.message ?? String(error ?? "Case refresh failed")
            return this.snapshot()
        }
